/*
	Copyright 2013-2014 Jens-Martin Loebel. 
...
function aboutReader() {
    $("#aboutDialog").dialog({
        ...
        buttons: {
            "Learn more...": function() {
                location.href = "http://hyperimage.sourceforge.net/";
            },
            "OK": function() {
                $(this).dialog("close");
            }
...
function loadItem(id, auxLoad, shouldSetGUI) {
    ...
    // do imprint handling
    if (id == 'imprint')
        if (reader.project.items['imprint'].content[reader.lang] == null) {
    ...
    if (reader.project.items[id] == null) {
        ...
        $.get(reader.path + id + '.xml', function(data, success) {
            parseItem(data, success, shouldSetGUI);
        });
    } else {
...
function getTitle(item) {
    ...
    if (item.type == 'object') title = item.md[reader.lang][
        [reader.project.sortedFields[1]]
    ];
    if (item.type == 'inscription') {
        var element = document.createElement("div");
        $(element).append(item.content[reader.lang]);
        title = $(element).text();
    }
                ...
                if (mode == null ? (word.substring(0, searchTerm.length) == searchTerm) : (word == searchTerm)) {
                    if (!advancedSearch) {
                        ...
                        $(Object.keys(reader.project.search[reader.lang].entries[word])).each(function(index, ref) {
                            ...
                            if (inKey && $.inArray(ref, results) < 0) results.push(ref);
            ...
            if (results.length > 0) {
                ...
                for (var i = 0; i < resultTerms.length; i++) {
                    if (resultTerms.length > 1)
                        if (i == (resultTerms.length - 1)) $('#searchResultsContent').append(' ' + andText + ' ');
...
function addViewToLightTable(viewID) {
    ...
    if (view == null) return;
    var frame = new HIFrame(20 + (16 * reader.table.frames.length), 66 + (16 * reader.table.frames.length), view.files['thumb'].width, view.files['thumb'].height);
    frame.href = view.id;
    frame.imagePos.x = 0;
    frame.imagePos.y = 0;
...
function duplicateFrame() {
    ...
    if (selectedFrame != null && !isAnnotation) {
        var frame = new HIFrame($('#lighttableContent').scrollLeft() + $(selectedFrame).position().left + 16, $('#lighttableContent').scrollTop() + $(selectedFrame).position().top + 32 + reader.zoom.yOffset, $(selectedFrame).width(), $(selectedFrame).height());
...
function fitToFrame() {
    ...
    $(img).css('top', '0px');
    $(img).css('left', '0px');
    $(img).width(view.files['original'].width * scale);
    $(img).height(view.files['original'].height * scale);
    $(selectedFrame).width(view.files['original'].width * scale);
    $(selectedFrame).height((view.files['original'].height * scale) + 16);
    window.syncTableToModel();
}
...
function fitToThumb() {
    ...
    $(img).css('top', '0px');
    $(img).css('left', '0px');
    $(img).width(view.files['thumb'].width);
    $(img).height(view.files['thumb'].height);
    $(selectedFrame).width(view.files['thumb'].width);
    $(selectedFrame).height(view.files['thumb'].height + 16);
    window.syncTableToModel();
}
...
function insertAnnotationLink() {
    ...
    var diagButtons = {};
    diagButtons[reader.strings[reader.lang]['STR_CANCEL']] = function() {
        $(this).dialog("close");
    };
...
function displayLightTable() {
    ...
    function syncTableToModel() {
        ...
        $(sortedFrames).each(function(index, item) {
            if ($(item).hasClass('ltAnnotation')) {
                ...
                var anString = '';
                $(serialized.split('<br>')).each(function(index, item) {
                    anString += item.trim() + '<br>';
                });
                ...
                var newFrame = new HIFrame($(item).position().left + $('#lighttableContent').scrollLeft(), $(item).position().top + $('#lighttableContent').scrollTop() + 64, $(item).width(), $(item).height() - 16);
    ...
    function setLightTableMenuGUI() {
        ...
        if (selectedFrame != null) {
            if (isAnnotation) {
                ...
                viewLink = $(selectedFrame).data('viewID');
        ...
        $('.infolita').text(reader.strings[reader.lang]['INFOLINE_LITA'] + ' ' + title);
    }
    ...
    function addFrame(item) {
        ...
        if (item == null) {
            ...
            $('#' + id).bind('contextmenu', function(ev) {
                ev.preventDefault();
            });
            ...
            $('#' + id + ' .ltText').bind('blur keyup paste', function() {
                window.syncTableToModel();
            });
            ...
            $('#' + id + "_content").dragscrollable({
                acceptPropagatedEvent: true
            });
            $('#' + id + "_content").bind('mouseup', 'vmouseup', function(e) {
                window.syncTableToModel();
            });
            var title = '';
        ...
        if (screen.width <= 1024) {
        	$('#' + id).on( "resize", function( e, ui ) {
	        	$('#' + id + "_content").dragscrollable('disable');
        	});
        	$('#' + id).on( "resizestop", function( event, ui ) {
	        	$('#' + id + "_content").dragscrollable('enable');
        	});
        	$(function(){
        		...
        		$.contextMenu('destroy', '.hasLitaContextMenu');
				$('.hasLitaContextMenu').off("taphold");
				$('.hasLitaContextMenu').on('taphold', function(e) {
					e.preventDefault();
					var viewID = $(this).data('viewID');
					console.log(viewID);
					$('#lita-context').css({top: holdCords.holdY, left: holdCords.holdX}).show();
					$("#lighttableContent > div.ltSelected").removeClass("ltSelected");
					$(this).addClass('ltSelected');
					setTimeout(hideContextmenu, 3000);
					$('#lita-view-select').one('click', function(e) {
						e.preventDefault();
						if ( viewID != null ) { 
					...
					$('#lita-fit-frame').one('click', function(e) {
						e.preventDefault();
						fitToFrame();
					});
					$('#lita-fit-thumb').one('click', function(e) {
						e.preventDefault();
						fitToThumb();
					});
					$('#lita-duplicate').off('click');
					$('#lita-duplicate').one('click', function(e) {
						e.preventDefault();
						duplicateFrame();
					});
					$('#lita-delete').off('click');
					$('#lita-delete').one('click', function(e) {
						e.preventDefault();
						removeFrame();
					});
         ...
         $('#' + id + '_content .ltText a').click(function(ev) {
            followLiTaLink(this);
        });
        $('#' + id).resizable({
            autoHide: true,
        ...
        $('#' + id + '_content img, #' + id + '_content .ltText, #' + id).on("mousedown", "vmousedown", function(e) {
            $("#lighttableContent > div.ltSelected").removeClass("ltSelected");
            $('#' + id).addClass('ltSelected');
            if ($('#' + id).hasClass('ltAnnotation')) $('#table_anno .ltText').focus();
            if ($('#' + id).hasClass('ltFrame')) $('#table_anno .ltText').blur();
            else $('#emptyInput').focus();
            var sortedFrames = $.makeArray($('.ltFrame')).sort(function(a, b) {
                return (parseInt($(a).css("zIndex"), 10) || 0) - (parseInt($(b).css("zIndex"), 10) || 0);
            });
            min = parseInt($(sortedFrames[0]).css("zIndex"), 10) || 0;
            $(sortedFrames).each(function(i) {
    ...
    if (reader.table.frameAnnotation.visible) addFrame();
...
function calcImageScales() {
    ...
    if (view == null) return;
    if (view.type == 'object') view = view.views[view.defaultViewID];
    else
    if (view.type == 'layer') view = view.parent;
    else
    ...
    if (view.type == 'inscription') {
        ...
        reader.zoom.mobile = Math.min(1.0 * ($('#canvas').width() / view.files['original'].width), ($('#canvas').height() - reader.zoom.yOffset) * 1.0 / view.files['original'].height);
                reader.zoom.mobile.width = Math.min(1.0 * ($('#canvas').width() / view.files['original'].width));
        if (screen.width <= 1024) {scaleImageTo(reader.zoom.mobile);}
    else
...
function zoomIn() {
    if (!$('#zoomInLink').parent().is('.disabled'))
        scaleImageTo(reader.zoom.cur * 1.5);
        if (screen.width <= 1024) {
        	if (reader.zoom.cur > reader.zoom.mobile) {
	        	$('.zoom-in').addClass('non');
...
function scaleImageTo(scale) {
    ...
    reader.canvas.svg.change(reader.canvas.svg.root(), {
        x: 0,
        y: 0,
        width: Math.round(reader.zoom.width * scale) + xPos,
        height: Math.round(reader.zoom.height * scale) + reader.zoom.yOffset + yPos
    });
...
function setZoomGUI(zoomInEnabled, zoomOutEnabled) {
    ...
    if (zoomInEnabled) $('#zoomInLink').parent().removeClass("disabled");
    else $('#zoomInLink').parent().addClass("disabled");
    if (zoomOutEnabled) $('#zoomOutLink').parent().removeClass("disabled");
    else $('#zoomOutLink').parent().addClass("disabled");
}
...
function showID() {
    ...
    switch (item.type) {
        ...
        case 'layer':
            ...
            break;
        case 'inscription':
            output = reader.strings[reader.lang]['ID_INSCRIPTION'] + ' ' + item.id;
            break;
        case 'text':
            output = reader.strings[reader.lang]['ID_TEXT'] + ' ' + item.id;
            break;
        case 'lita':
            output = reader.strings[reader.lang]['ID_LITA'] + ' ' + item.id;
            break;
    }
...
function highlightLayer(id) {
    hideAllLayers();
    $('#' + id + '_group').attr("stroke-width", 3 * (1 / reader.zoom.cur)).fadeTo(1, 0).delay(250).fadeTo(1, 1).delay(250).fadeTo(1, 0).delay(250).fadeTo(1, 1).delay(250).fadeTo(1, 0).delay(250).fadeTo(1, 1).delay(250).fadeTo(1, 0).delay(250).fadeTo(1, 1);
}
...
function displayViewMetadata(view) {
    ...
    if (view.title[reader.lang] != null && view.title[reader.lang].length > 0) {
        ...
        if (reader.fromSearch)
            $(reader.search.resultTerms).each(function(index, term) {
                viewTitle = highlightWord(viewTitle, term);
            });
    ...
    if (view.source[reader.lang] != null && view.source[reader.lang].length > 0) {
        ...
        if (reader.fromSearch)
            $(reader.search.resultTerms).each(function(index, term) {
                viewSource = highlightWord(viewSource, term);
            });
...
function displayLayerMetadata(layer) {
    ...
    if (layer.annotation[reader.lang] != null && layer.annotation[reader.lang].length > 0) {
        ...
        if (reader.fromSearch)
            $(reader.search.resultTerms).each(function(index, term) {
                layerAnnotation = highlightWord(layerAnnotation, term);
            });
...
function displayInscription(inscription) {
    ...
    if (reader.fromSearch)
        $(reader.search.resultTerms).each(function(index, term) {
            insContent = highlightWord(insContent, term);
        });
...
function setMenuState() {
    ...
    if ($('#lighttableView').is(':visible')) {
        $('#displayMenu li').each(function(index, item) {
            if (!$(item).hasClass('disabled')) $(item).addClass('disabled');
        });
        ...
        setMenuItem('addAnnotationLink_ON', 'javascript:void(0);', false);
        setMenuItem('addAnnotationLink_OFF', 'javascript:void(0);', false);
...
function setGUIMode(mode) {
    ...
    if (mode == 3) {
        $('#sidebar').hide();
        $('#tabs').hide();
        $('#info').css('background-color', reader.prefs['LITA_INFOLINE_COLOR']);
        $('#info').css('color', reader.prefs['LITA_INFO_COLOR']);
        $('#info').css('font-family', reader.prefs['LITA_INFO_FONT']);
        $('#info').css('font-size', reader.prefs['LITA_INFO_SIZE'] + 'px');
    } else {
        if (screen.width <= 1024) {
        ...
        $('#info').css("background-color", reader.prefs['INFOLINE_COLOR']);
        $('#info').css("color", reader.prefs['MAINTEXT_COLOR']);
        $('#info').css('font-family', reader.prefs['MAINTEXT_FONT']);
        $('#info').css('font-size', reader.prefs['MAINTEXT_SIZE'] + 'px');
    }
}
...
function setGUI(forceLoad) {
    ...
    if (getObject(item) != null)
        if (item.type == 'view') newViewID = item.id;
        else
    if (item.type == 'layer') newViewID = item.parent.id;
    else newViewID = getObject(item).defaultViewID;
    ...
    $("#viewannotation_field").hide();
    $("#groupannotation_field").hide();
    $("#layerannotation_field").hide();
    $("#viewtitle_field").hide();
    $("#viewsource_field").hide();
    ...
    switch (guiMode) {
        ...
        case 1:
            ...
            if (reader.fromSearch)
                $(reader.search.resultTerms).each(function(index, term) {
                    textContent = highlightWord(textContent, term);
                });
            $('#textContent').html(textContent);
            break;
        case 2:
            if (reader.mode == 'refs') {
                if (item.refs != null) list = item.refs;
                else list = reader.project.items[reader.viewID].refs;
            } else if (reader.mode == 'sites') {
                if (item.sites != null) list = item.sites;
                else list = reader.project.items[reader.viewID].sites;
                ...
                if (item.annotation[reader.lang] != null && item.annotation[reader.lang].length > 0) {
                    ...
                    if (reader.fromSearch)
                        $(reader.search.resultTerms).each(function(index, term) {
                            groupAnnotation = highlightWord(groupAnnotation, term);
                        });
        ...
        case 3:
            reader.viewID = null;
            reader.table = JSON.parse(JSON.stringify(item));
            displayLightTable();
            break;
    }
    ...
    if (reader.mode == 'regular')
        if (item.type == 'object' || item.type == 'view' || item.type == 'layer') {
            ...
            if (obj.md[reader.lang] != null) {
                ...
                if (reader.prefs['INFOLINE_METADATA_VIEW'] && reader.project.items[reader.viewID] != null && reader.project.items[reader.viewID].title[reader.lang] != null && reader.project.items[reader.viewID].title[reader.lang].length > 0) {
    ...
    if (loadViewFile || (forceLoad && guiMode == 0)) {
		...
		*/
        var view = reader.project.items[reader.viewID];
        $('#canvasLayerGroup > g').remove();
        $("#canvasImage").attr('width', view.files['original'].width);
        $("#canvasImage").attr('height', view.files['original'].height);
        loadViewFileData($("#canvasImage"), view, view.files['images'][0].href);
        $('#canvas').scrollTop(0);
        $('#canvas').scrollLeft(0);	
        calcImageScales();
        scaleImageTo(reader.zoom.image);
        loadHiResIfNeeded(view);
        findHigherRes(view);
        if (screen.width <= 1024) { 
        scaleImageTo(Math.min(1.0 * ($('#canvas').width() / view.files['original'].width), ($('#canvas').height() - reader.zoom.yOffset) * 1.0 / view.files['original'].height));loadHiResIfNeeded(view);findHigherRes(view);
        }
        // draw layers
		$(view.sortedLayers).each(function(index, layer) {
			...
			if (screen.width <= 1024) {
				...
				$(function(){
					$('#canvasTooltip').css('top', '100px');
                    $('#canvasTooltip').css('left', '0px');
            		$.contextMenu('destroy', '[id$=_group]');
					$('[id$=_group]').off("taphold");
					$('[id$=_group]').on('taphold', function(e) {
