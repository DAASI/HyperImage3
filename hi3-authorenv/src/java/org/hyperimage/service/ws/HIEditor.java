...
public class HIEditor {
	...
	private PRrest prometheusAPI = null; // Prometheus OAUTH API --> only set if
	...
	/**
	 * Constructor for OAUTH / Prometheus users - should only be instantiated by
	 * HILogin WebService
	 *
	 * @param curUser
	 *            logged in user, authenticated and passed on by the HILogin
	 * @param prAPI
	 *            prometheus API class containing all necessary OAUTH tokens
	 *            WebService
	 ...
	 *
	 * @param userID
	 *            user ID for role lookup
	 * @param projectID
	 *            project ID for role lookup
	 * @return role for given user in given project
	 * @throws HIServiceException
	 *             thrown if no project is selected (when invoked by admin) or
	 *             the given user does not belong to the given project
	 * @throws HIPrivilegeException
	 *             thrown if invoked by admin with different project ID than the
	 *             current project or if invoked by regular user or guest.
	 * @throws HIEntityNotFoundException
	 *             thrown if user ID or project ID could not be found
	 * @throws HIMaintenanceModeException
	 *             thrown if HIEditor WebService is in maintenance mode
	 */
	@WebMethod
	public HIRole.HI_Roles adminGetRoleInProject(@WebParam(name = "userID") long userID,
			@WebParam(name = "projectID") long projectID) throws HIServiceException, HIPrivilegeException,
					HIEntityNotFoundException, HIMaintenanceModeException {
	...
	@WebMethod
	public boolean setProject(@WebParam(name = "project") HIProject project)
		...
		// check if user is a member of desired project, if so retrieve role, if
		// not --> abort
		HIRole role = null;
		if (!isSysop()) {
		...
		try {
			...
			importGroup = (HIGroup) em.createQuery("SELECT g from HIGroup g WHERE g.project=:project AND g.type=:type")
					.setParameter("project", curProject).setParameter("type", HIGroup.GroupTypes.HIGROUP_IMPORT)
					.getSingleResult();
			...
			trashGroup = (HIGroup) em.createQuery("SELECT g from HIGroup g WHERE g.project=:project AND g.type=:type")
					.setParameter("project", curProject).setParameter("type", HIGroup.GroupTypes.HIGROUP_TRASH)
					.getSingleResult();
		...
		// log all actions
		/*
		 * List<HIBase> contents = em.createQuery(
		 * "SELECT b from HIBase b WHERE b.project=:project")
		 * .setParameter("project", curProject) .getResultList(); m_logger.info(
		 * "Project P"+curProject.getId()+": contains " + contents.size() +
		 * " elements\n");
		 * 
		 * int lostContent = 0; for ( HIBase content : contents ) { // filter
		 * out object content if ( !(content instanceof HILayer) && !(content
		 * instanceof HIView) && !(content instanceof HIInscription) &&
		 * !(content instanceof HIGroup) ) {
		 * 
		 * // count groups for base long size = (Long) em.createQuery(
		 * "SELECT count(g) FROM HIGroup g WHERE g.contents=:base AND NOT g.type=:type"
		 * ) .setParameter("base", content) .setParameter("type",
		 * HIGroup.GroupTypes.HIGROUP_TAG) .getSingleResult();
		 * 
		 * // found orphaned content if ( size < 1 ) { lostContent = lostContent
		 * + 1; m_logger.info("Project P"+curProject.getId()+
		 * ": found orphaned content ID:"+content.getId()+" Type:"
		 * +content.getClass()); // add content back to import group if (
		 * addToImportGroup(content) ) m_logger.info("Project P"
		 * +curProject.getId()+": content successfully added to import group!");
		 * else m_logger.info("Project P"+curProject.getId()+
		 * ": FAILED to add content to import group!"); } } } if ( lostContent
		 * == 0 ) m_logger.info("Project P"+curProject.getId()+
		 * ": No orphaned content found."); else m_logger.info("Project P"
		 * +curProject.getId()+": found "+lostContent+" lost elements.");
		 */
		return true;
	}
	...
	private HIProject createPRProject(PRCollection collection) {
		...
		HILanguage defLang = new HILanguage("de"); // German is the only
													// supported language of
													// Prometheus
		...
		try {
			...
			HIProjectMetadata projMetadata = new HIProjectMetadata(defLang.getLanguageId(), collection.getTitle(),
					project);
	...
	/*
	 * List projects for Prometheus users. Only displays projects initially
	 * created via Prometheus
	 */
	private List<HIProject> getProjectsPR() throws HIMaintenanceModeException {
		...
		EntityManager em = emf.createEntityManager();
		projects = (List<HIProject>) em.createQuery(query).setParameter("user", curUser).getResultList();

		// get list of Prometheus collections
		List<PRCollection> prCollections = prometheusAPI.getCollections();

		// check if new collections were created in Prometheus --> create
		// corresponding HI projects
			...
			for (PRCollection collection : prCollections) {
				...
				for (HIProject project : projects)
					if (findPreferenceValue(project.getPreferences(), "PRCollectionID") != null
							&& findPreferenceValue(project.getPreferences(), "PRCollectionID")
									.compareTo(collection.getID()) == 0)
						found = true;
				if (!found)
		...
		for (PRCollection collection : newCollections) {
			...
			if (newProject != null)
				projects.add(em.find(HIProject.class, newProject.getId()));
		}
	...
	// -- SYSOP METHOD, a user gets a list of all his projects, the sysop gets
	// all projects in the database --
	@SuppressWarnings("unchecked")
	@WebMethod
	public List<HIProject> getProjects() throws HIMaintenanceModeException {

		if ((prometheusAPI != null))
			return getProjectsPR(); // display projects for Prometheus/OAUTH
									// user
		...
		if (isSysop()) {
			...
			projects = (List<HIProject>) em.createQuery(query).getResultList();
		} else {
			projects = (List<HIProject>) em.createQuery(query).setParameter("user", curUser).getResultList();
		}
	...
	// -- SYSOP METHOD --
	@WebMethod
	public synchronized HIUser sysopCreateUser(@WebParam(name = "firstName") String firstName,
			@WebParam(name = "lastName") String lastName, @WebParam(name = "userName") String userName,
	...
	// -- ADMIN METHOD --
	@WebMethod
	public HIUser adminGetUserByID(@WebParam(name = "userID") long userID)
	...
	// -- ADMIN METHOD --
	@WebMethod
	public HIUser adminGetUserByUserName(@WebParam(name = "userName") String userName)
	...
	@WebMethod
	public synchronized boolean updateUser(@WebParam(name = "userID") long userID,
			@WebParam(name = "firstName") String firstName, @WebParam(name = "lastName") String lastName,
			@WebParam(name = "email") String email) throws HIParameterException, HIEntityNotFoundException,
					HIPrivilegeException, HIMaintenanceModeException {
		...
		// a user may only edit his own record, only the sysop can edit every
		// user�s profile
	...
	// -- SYSOP METHOD, however a user may edit his own profile --
	@WebMethod
	public synchronized boolean updateUserPassword(@WebParam(name = "userID") long userID,
			@WebParam(name = "password") String password) throws HIParameterException, HIEntityNotFoundException,
					HIPrivilegeException, HIMaintenanceModeException {
	...
	@SuppressWarnings("unchecked")
	@WebMethod
	public synchronized boolean sysopDeleteUser(@WebParam(name = "userID") long userID)
		...
		List<HIRole> roles = em.createQuery("SELECT r FROM HIRole r WHERE r.user=:user").setParameter("user", user)
				.getResultList();
		try {
			utx.begin();
			em.joinTransaction();
	...
	@SuppressWarnings("unchecked")
	@WebMethod
	public synchronized List<HIProject> sysopGetProjectsForUser(@WebParam(name = "userID") long userID)
			throws HIEntityNotFoundException, HIMaintenanceModeException {
	...
	// -- ADMIN METHOD --
	@WebMethod
	public synchronized boolean adminSetProjectRole(@WebParam(name = "userID") long userID,
		...
		try {
			projectRole = (HIRole) em.createQuery("SELECT r from HIRole r WHERE r.user=:user AND r.project=:project")
					.setParameter("user", user).setParameter("project", curProject).getSingleResult();
		} catch (NoResultException e) {
			throw new HIPrivilegeException("User does not belong to current project!");
		}
	...
	// --
	@WebMethod
	public synchronized boolean adminAddUserToProject(@WebParam(name = "userID") long userID,
			@WebParam(name = "role") HIRole.HI_Roles role) throws HIServiceException, HIMaintenanceModeException {
	...
	// -- ADMIN METHOD --
	@WebMethod
	public synchronized boolean adminRemoveUserFromProject(@WebParam(name = "userID") long userID)
			throws HIPrivilegeException, HIServiceException, HIMaintenanceModeException {
	...
	@WebMethod
	public synchronized boolean sysopAddUserToProject(@WebParam(name = "userID") long userID,
			@WebParam(name = "projectID") long projectID, @WebParam(name = "role") HIRole.HI_Roles role)
		...
		try {
			projectRole = (HIRole) em.createQuery("SELECT r from HIRole r WHERE r.user=:user AND r.project=:project")
					.setParameter("user", user).setParameter("project", project).getSingleResult();
		} catch (NoResultException e) {
		}
		try {
			utx.begin();
	...
	// -- SYSOP METHOD --
	@WebMethod
	public synchronized boolean sysopRemoveUserFromProject(@WebParam(name = "userID") long userID,
	...
	@SuppressWarnings("unchecked")
	@WebMethod
	public synchronized List<HIUser> sysopGetUsersForProject(@WebParam(name = "projectID") long projectID)
			throws HIEntityNotFoundException, HIMaintenanceModeException {
		...
		users = (List<HIUser>) em.createQuery(query).setParameter("project", project).getResultList();
	...
	public synchronized List<HIUser> adminGetProjectUsers()
		...
		String query = "SELECT r.user FROM HIRole r WHERE r.project=:project";
		users = (List<HIUser>) em.createQuery(query).setParameter("project", curProject).getResultList();
	...
	// -- SYSOP METHOD --
	@WebMethod
	public synchronized HIProject sysopCreateProject(@WebParam(name = "adminUsername") String adminUsername,
		...
		try {
			projectAdmin = (HIUser) em.createQuery("SELECT u from HIUser u WHERE u.userName=:username")
					.setParameter("username", adminUsername).getSingleResult();
		} catch (NoResultException e) {
			throw new HIEntityNotFoundException("User not found!");
		}
	...
	@SuppressWarnings("unchecked")
	@WebMethod
	public synchronized boolean adminAddLanguageToProject(@WebParam(name = "language") String language)
			throws HIServiceException, HIMaintenanceModeException {
	...
	@SuppressWarnings("unchecked")
	@WebMethod
	public synchronized boolean adminRemoveLanguageFromProject(@WebParam(name = "language") String language)
			throws HIServiceException, HIMaintenanceModeException {
				...
				for (HIBase base : projectEntities) {
					if (!(base instanceof HIURL) && !(base instanceof HILightTable)) {
						...
						if (langIndex >= 0) {
							...
							em.remove(record);
							// update lucene index
							indexer.storeElement(base, curProject); // update
																	// lucene
																	// index
						}
					}
	...
	@SuppressWarnings("unchecked")
	@WebMethod
	public synchronized boolean adminAddTemplateToProject(@WebParam(name = "template") HIFlexMetadataTemplate template)
			throws HIServiceException, HIMaintenanceModeException {
	...
	@WebMethod
	public synchronized boolean adminUpdateTemplate(@WebParam(name = "templateID") long templateID,
			@WebParam(name = "templateURI") String templateURI, @WebParam(name = "templateURL") String templateURL)
					throws HIMaintenanceModeException, HIEntityNotFoundException, HIPrivilegeException,
					HIParameterException {
	...
	@WebMethod
	public synchronized boolean adminUpdateTemplateSortOrder(@WebParam(name = "templateID") long templateID,
			@WebParam(name = "sortOrder") String sortOrder) throws HIMaintenanceModeException,
					HIEntityNotFoundException, HIPrivilegeException, HIParameterException {
	...
	@WebMethod
	public synchronized boolean adminRemoveTemplateFromProject(@WebParam(name = "templateID") long templateID)
	...
	@WebMethod
	public synchronized HIFlexMetadataSet adminAddSetToTemplate(@WebParam(name = "templateID") long templateID,
			@WebParam(name = "tagName") String tagName, @WebParam(name = "isRichText") boolean isRichText)
					throws HIMaintenanceModeException, HIEntityNotFoundException, HIEntityException,
					HIPrivilegeException, HIParameterException {
			...
			try {
				...
				for (HIObject object : objects) {
					...
					for (HILanguage lang : curProject.getLanguages()) {
						...
						if (record != null) {
							record.addEntry(
									new HIKeyValue(template.getNamespacePrefix() + "." + newSet.getTagname(), ""));
							em.persist(record);
							em.flush();
						}
					}
	...
	@WebMethod
	public synchronized HIFlexMetadataName adminCreateSetDisplayName(@WebParam(name = "setID") long setID,
			@WebParam(name = "languageID") String languageID, @WebParam(name = "displayName") String displayName)
					throws HIMaintenanceModeException, HIParameterException, HIEntityNotFoundException,
					HIPrivilegeException {
	...
	@WebMethod
	public synchronized void adminUpdateSetDisplayName(@WebParam(name = "setID") long setID,
			@WebParam(name = "languageID") String languageID, @WebParam(name = "displayName") String displayName)
					throws HIMaintenanceModeException, HIParameterException, HIEntityNotFoundException,
					HIPrivilegeException {
	...
	@WebMethod
	public synchronized boolean adminRemoveSetFromTemplate(@WebParam(name = "templateID") long templateID,
			@WebParam(name = "setID") long setID) throws HIMaintenanceModeException, HIParameterException,
					HIPrivilegeException, HIEntityNotFoundException {
	...
	@WebMethod
	public synchronized boolean updateProjectMetadata(@WebParam(name = "languageID") String languageID,
			@WebParam(name = "title") String title) throws HIPrivilegeException, HIMaintenanceModeException {
	...
	@WebMethod
	public synchronized boolean updateProjectStartElement(@WebParam(name = "baseID") long baseID)
	...
	@WebMethod
	public synchronized boolean updateProjectDefaultLanguage(@WebParam(name = "language") String language)
	...
	@WebMethod
	public synchronized HIPreference createPreference(@WebParam(name = "key") String key,
			@WebParam(name = "value") String value) throws HIParameterException, HIMaintenanceModeException {
	...
	@WebMethod
	public synchronized boolean updatePreference(@WebParam(name = "prefID") long prefID,
			@WebParam(name = "value") String value) throws HIParameterException, HIEntityNotFoundException,
					HIPrivilegeException, HIMaintenanceModeException {
	...
	@WebMethod
	public synchronized boolean deletePreference(@WebParam(name = "prefID") long prefID)
	...
	private void updatePRAssets() {
		List<PRImage> prImages = prometheusAPI
				.getCollectionImageInfo(MetadataHelper.findPreferenceValue(curProject, "PRCollectionID"));
		if (prImages == null)
			return;
		...
		if (hiPIDs.length() > 0)
			hiImages.addAll(Arrays.asList(hiPIDs.split("\\|"))); // gather
																	// project
																	// PIDs
		// compile list of new prometheus images that need to be uploaded to HI
		for (PRImage image : prImages)
			if (!hiImages.contains(image.getPID()))
				newImages.add(image);

		// upload new images to HI project
		for (PRImage image : newImages) {
			try {
				image.setData(prometheusAPI.getImageData(image.getPID(), "large")); // get
																					// image
																					// from
																					// prometheus
				HIObject hiObject = createObject(null);
				// attach Prometheus metadata
				HIFlexMetadataRecord record = MetadataHelper.getDefaultMetadataRecord(hiObject,
						curProject.getDefaultLanguage().getLanguageId());
				...
				HIView hiView = createView(hiObject.getId(),
						image.getPID() + ".jpg", "Prometheus [Collection "
								+ MetadataHelper.findPreferenceValue(curProject, "PRCollectionID") + "]",
						image.getData(), UUID.nameUUIDFromBytes(image.getData()).toString());

				if (hiView == null) {
					moveToTrash(hiObject.getId());
					deleteFromProject(hiObject.getId());
				} else
					hiImages.add(image.getPID());
		...
		// TODO delete images from HI project that have been removed in
		// Prometheus

		// serialize PIDs to HI Project
		String pidString = "";
		for (String pid : hiImages)
			pidString += "|" + pid;
		if (pidString.startsWith("|"))
			pidString = pidString.substring(1);
	...
	public synchronized HIGroup getImportGroup() throws HIMaintenanceModeException {
		...
		if ((prometheusAPI != null))
			updatePRAssets(); // for OAUTH / Prometheus users import/update
								// assets from Prometheus
	...
	@WebMethod
	public synchronized boolean deleteFromProject(@WebParam(name = "baseID") long baseID)
			throws HIMaintenanceModeException {
	...
	// -- SYSOP METHOD --
	@WebMethod
	public synchronized void sysopUpdateProjectQuota(@WebParam(name = "projectID") long projectID,
			@WebParam(name = "quota") long quota) throws HIPrivilegeException, HIEntityNotFoundException,
					HIParameterException, HIMaintenanceModeException {
	...
	@SuppressWarnings("unchecked")
	@WebMethod
	public synchronized boolean sysopDeleteProject(@WebParam(name = "projectID") long projectID)
		...
		try {
			...
			List<HIRepository> repositories = em
					.createQuery("SELECT rep FROM HIRepository rep WHERE rep.project=:project")
			...
			List<HILayer> layers = em
					.createQuery("SELECT l FROM HILayer l WHERE l.view.project=:project AND l.link is NOT NULL")
					.setParameter("project", project).getResultList();
			...
			List<HIRole> roles = (List<HIRole>) em.createQuery(query).setParameter("project", project).getResultList();
			...
			// find all project languages
			query = "SELECT l FROM HILanguage l WHERE l.project=:project";
			List<HILanguage> languages = (List<HILanguage>) em.createQuery(query).setParameter("project", project).getResultList();
			// delete all project languages
			for (HILanguage language : languages) {
				em.remove(language);
				em.flush();
			}
	...
	@WebMethod
	public synchronized List<HIText> getProjectTextElements() throws HIMaintenanceModeException, HIPrivilegeException {
	...
	public synchronized HIBase getBaseElementByUUID(@WebParam(name = "uuid") String uuid)
		...
		if (!isValidUUID(uuid))
			throw new HIParameterException("Invalid UUID format!");
		...
		try {
			base = (HIBase) em
					.createQuery("SELECT base FROM HIBase base WHERE base.project=:project AND base.uuid=:uuid")
					.setParameter("project", curProject).setParameter("uuid", uuid).getSingleResult();
		} catch (NoResultException e) {
			base = null;
		}
			...
			try {
				...
				for (HILanguage lang : curProject.getLanguages()) {
					...
					for (HIFlexMetadataRecord extraRecord : object.getMetadata()) {
						if (extraRecord.getLanguage().compareTo(lang.getLanguageId()) == 0
								&& extraRecord.getId() != rec.getId()) {
							extraRecords.addElement(extraRecord); // add to
																	// vector
																	// for later
																	// deletion
	...
	@WebMethod
	public synchronized HIBase getBaseElement(@WebParam(name = "baseID") long baseID)
			throws HIPrivilegeException, HIEntityNotFoundException, HIMaintenanceModeException {
		...
		if (needsUUIDUpgrade) {
			try {
				...
				if (base instanceof HIView) {
					if (base.getUUID() == null)
						base.setUUID(UUID.nameUUIDFromBytes(storageManager.getOriginal((HIView) base)).toString());
					for (HILayer layer : ((HIView) base).getLayers()) {
						if (layer.getUUID() == null)
							layer.setUUID(UUID.randomUUID().toString());
					}
				} else if (base instanceof HIObject) {
					if (base.getUUID() == null)
					...
					for (HIObjectContent content : ((HIObject) base).getViews()) {
						if (content instanceof HIView) {
							if (content.getUUID() == null)
								content.setUUID(UUID.nameUUIDFromBytes(storageManager.getOriginal((HIView) content))
										.toString());
							for (HILayer layer : ((HIView) content).getLayers()) {
								if (layer.getUUID() == null)
									layer.setUUID(UUID.randomUUID().toString());
							}
						} else if (content.getUUID() == null)
							content.setUUID(UUID.randomUUID().toString());
					}
				} else if (base.getUUID() == null)
					base.setUUID(UUID.randomUUID().toString());
	...
	// -------------------------
	@WebMethod
	public synchronized HIObject createObject(@WebParam(name = "uuid") String uuid)
	...
	@WebMethod
	public synchronized void updateObjectSortOrder(@WebParam(name = "objectID") long objectID,
			@WebParam(name = "sortOrder") String sortOrder) throws HIEntityNotFoundException, HIPrivilegeException,
					HIParameterException, HIMaintenanceModeException {
	...
	// -------------------------
	@WebMethod
	public synchronized HIView createView(@WebParam(name = "objectID") long objectID,
			@WebParam(name = "filename") String filename,
			@WebParam(name = "repositoryIDString") String repositoryIDString, @WebParam(name = "data") byte[] data,
			@WebParam(name = "uuid") String uuid) throws HIParameterException, HIEntityNotFoundException,
					HIPrivilegeException, HIEntityException, HIMaintenanceModeException {

		m_logger.fine(String.format(
				"createView called with objectID %s, filename %s, repositoryIDString %s, data %s, uuid %s", objectID,
				filename, repositoryIDString, data, uuid));
		if (filename.endsWith(".svg")) {
			m_logger.finest(String.format("SVG: %s", new String(data)));
		}
		...
		if (curProject.getQuota() > 0 && (curProject.getUsed() + data.length) > curProject.getQuota())
			throw new HIPrivilegeException(("Quota exceeded for project! Max storage: " + curProject.getQuota()
					+ " - Used: " + curProject.getUsed()));
		...
		long count = (Long) em.createQuery("SELECT COUNT(v) FROM HIView v WHERE v.hash=:hash and v.project=:project")
				.setParameter("hash", hash).setParameter("project", curProject).getSingleResult();
...
//			throw new HIEntityException("View already in project!");
	...
	@WebMethod
	public synchronized boolean setDefaultView(@WebParam(name = "objectID") long objectID,
	...
	@WebMethod
	public synchronized boolean updateContentOwner(@WebParam(name = "objectID") long objectID,
			@WebParam(name = "contentID") long contentID)
					throws HIEntityNotFoundException, HIPrivilegeException, HIMaintenanceModeException {
	...
	@WebMethod
	public synchronized void updateViewSortOrder(@WebParam(name = "viewID") long viewID,
			@WebParam(name = "sortOrder") String sortOrder) throws HIEntityNotFoundException, HIPrivilegeException,
					HIParameterException, HIMaintenanceModeException {
	...
	public synchronized byte[] getImage(@WebParam(name = "baseID") long baseID, HI_ImageSizes size)
		...
		if (base instanceof HIView) {
			view = (HIView) base;
		} // if base was an object try to find the default view, else take the
			// first view
		else if (base instanceof HIObject) {
			HIObject object = (HIObject) base;
			...
			// if base was layer find parent view and manually render layer on
			// top of it
			view = ((HILayer) base).getView();
		}

		if (view == null) {
			m_logger.severe("No view found. Returning null.");
			return null;
		}
		...
		if (size == HI_ImageSizes.HI_THUMBNAIL) {
			...
			if (mimetype == null || !mimetype.startsWith("image/")) {
				...
				// try {
				// bitstream =
				// FileStorageManager.getBytesFromFile(getClass().getResource("/resources/no-thumbnail.png").getFile());
				m_logger.severe(
						String.format("No image, nothing to retrieve (File name: %s, mime type: %s). Returning null.",
								filename, mimetype));
				return bitstream;
				// } catch (IOException e) {
	...
	/*
	 * Returns image scaled to custom resolution. This image is generated
	 * on-the-fly and not stored in HIStore.
	 */
	@WebMethod
	public synchronized byte[] getScaledImage(@WebParam(name = "baseID") long baseID, double scale)
			throws HIPrivilegeException, HIEntityNotFoundException, HIMaintenanceModeException {
		...
		// BufferedImage previewImage =
		// ImageHelper.convertByteArrayToBufferedImage(storageManager.getHiRes(view));
		BufferedImage previewImage = HiImageConfig.getHiImage()
				.createImageFromStream(new ByteArrayInputStream(storageManager.getHiRes(view)));
		// scale image back to thumbnail size
		// previewImage = ImageHelper.scaleImageTo(previewImage, new
		// Dimension((int)Math.round(view.getWidth()*scale),
		// (int)Math.round(view.getHeight()*scale))).getAsBufferedImage();
		previewImage = HiImageConfig.getHiImage().scaleImage(previewImage,
				new Dimension((int) Math.round(view.getWidth() * scale), (int) Math.round(view.getHeight() * scale)),
				view.getRepositoryID());
		...
		ByteArrayOutputStream outImage = HiImageConfig.getHiImage().convertToStream(previewImage);
	...
	// -------------------------
	@WebMethod
	public synchronized HIInscription createInscription(@WebParam(name = "objectID") long objectID,
			@WebParam(name = "uuid") String uuid) throws HIPrivilegeException, HIEntityNotFoundException,
					HIMaintenanceModeException, HIParameterException, HIEntityException {
	...
	// -------------------------
	@WebMethod
	public synchronized HILayer createLayer(@WebParam(name = "viewID") long viewID, @WebParam(name = "red") int red,
			@WebParam(name = "green") int green, @WebParam(name = "blue") int blue,
			@WebParam(name = "opacity") float opacity, @WebParam(name = "uuid") String uuid)
					throws HIPrivilegeException, HIEntityNotFoundException, HIMaintenanceModeException,
					HIParameterException, HIEntityException {
	...
	@WebMethod
	public synchronized boolean updateLayerProperties(@WebParam(name = "layerID") long layerID,
			@WebParam(name = "red") int red, @WebParam(name = "green") int green, @WebParam(name = "blue") int blue,
			@WebParam(name = "opacity") float opacity, @WebParam(name = "polygons") String polygons)
					throws HIMaintenanceModeException {
	...
	@WebMethod
	public synchronized boolean removeLayer(@WebParam(name = "layerID") long layerID)
			throws HIMaintenanceModeException {
	...
	@WebMethod
	public synchronized boolean setLayerLink(@WebParam(name = "layerID") long layerID,
			@WebParam(name = "linkID") long linkID) throws HIParameterException, HIPrivilegeException,
					HIEntityNotFoundException, HIMaintenanceModeException {
		...
		// check if link element is in trash --> can�t link to things in the
		// trash
		trashGroup = em.find(HIGroup.class, trashGroup.getId());
		if (trashGroup == null) {
	...
	@WebMethod
	public synchronized boolean removeLayerLink(@WebParam(name = "layerID") long layerID)
	...
	// -------------------------
	@WebMethod
	public synchronized HIURL createURL(@WebParam(name = "url") String url, @WebParam(name = "title") String title,
			@WebParam(name = "lastAccess") String lastAccess, @WebParam(name = "uuid") String uuid)
	...
	@WebMethod
	public synchronized boolean updateURL(@WebParam(name = "hiURL") HIURL hiURL)
			throws HIParameterException, HIPrivilegeException, HIEntityNotFoundException, HIMaintenanceModeException {
	...
	// -------------------------
	@WebMethod
	public synchronized HILightTable createLightTable(@WebParam(name = "title") String title,
			@WebParam(name = "xml") String xml, @WebParam(name = "uuid") String uuid)
					throws HIParameterException, HIMaintenanceModeException, HIEntityException {
	...
	@WebMethod
	public synchronized boolean updateLightTable(@WebParam(name = "lightTable") HILightTable lightTable)
			throws HIParameterException, HIPrivilegeException, HIEntityNotFoundException, HIMaintenanceModeException {
	...
	public synchronized List<HIGroup> getTagGroups() throws HIMaintenanceModeException {
		...
		tagGroups = em.createQuery("SELECT g FROM HIGroup g WHERE g.project=:project AND g.type=:type")
				.setParameter("project", curProject).setParameter("type", HIGroup.GroupTypes.HIGROUP_TAG)
				.getResultList();
	...
	@WebMethod
	public synchronized HIGroup createTagGroup(@WebParam(name = "uuid") String uuid)
			throws HIMaintenanceModeException, HIEntityException, HIParameterException {
	...
	@WebMethod
	public synchronized List<Long> getTagIDsForBase(@WebParam(name = "baseID") long baseID)
			throws HIEntityNotFoundException, HIPrivilegeException {
		List<Long> tagIDList;
		...
		tagIDList = em
				...
				.setParameter("project", curProject).setParameter("type", HIGroup.GroupTypes.HIGROUP_TAG)
				.setParameter("base", base).getResultList();
	...
	@WebMethod
	public synchronized List<Long> getTagIDsForBaseUUID(@WebParam(name = "uuid") String uuid)
			throws HIParameterException, HIEntityNotFoundException, HIPrivilegeException {
		if (uuid != null && !isValidUUID(uuid))
			throw new HIParameterException("Invalid UUID format!");
		long baseID = getBaseElementByUUID(uuid).getId(); // check if base
															// exists and get
															// internal id
	...
	@WebMethod
	public synchronized long getTagCountForBase(@WebParam(name = "baseID") long baseID)
			throws HIEntityNotFoundException, HIPrivilegeException {
		HIBase base = getBaseElement(baseID); // check if base exists
		...
		return (Long) em
				...
				.setParameter("project", curProject).setParameter("type", HIGroup.GroupTypes.HIGROUP_TAG)
				.setParameter("base", base).getSingleResult();
	}

	@WebMethod
	public synchronized long getTagCountForBaseUUID(@WebParam(name = "uuid") String uuid)
			throws HIParameterException, HIEntityNotFoundException, HIPrivilegeException {
		if (uuid != null && !isValidUUID(uuid))
			throw new HIParameterException("Invalid UUID format!");
		long baseID = getBaseElementByUUID(uuid).getId(); // check if base
	...
	public synchronized List<HIGroup> getGroups() throws HIMaintenanceModeException {
		...
		groups = em.createQuery("SELECT g FROM HIGroup g WHERE g.project=:project AND g.type=:type")
				.setParameter("project", curProject).setParameter("type", HIGroup.GroupTypes.HIGROUP_REGULAR)
				.getResultList();
	...
	@WebMethod
	public synchronized HIGroup createGroup(@WebParam(name = "uuid") String uuid)
			throws HIMaintenanceModeException, HIEntityException, HIParameterException {
	...
	@WebMethod
	@Deprecated
	public synchronized boolean updateGroupProperties(@WebParam(name = "groupID") long groupID,
	...
	@WebMethod
	public synchronized void updateGroupSortOrder(@WebParam(name = "groupID") long groupID,
			@WebParam(name = "sortOrder") String sortOrder) throws HIEntityException, HIEntityNotFoundException,
					HIPrivilegeException, HIParameterException, HIMaintenanceModeException {
	...
	@WebMethod
	public synchronized boolean deleteGroup(@WebParam(name = "groupID") long groupID)
		...
		for (HIBase content : group.getContents()) {
			if (!(content instanceof HIView) && !(content instanceof HIInscription) && !(content instanceof HILayer)) {
	...
	@WebMethod
	public synchronized List<HIQuickInfo> getGroupContentQuickInfo(@WebParam(name = "groupID") long groupID)
			throws HIPrivilegeException, HIEntityNotFoundException, HIMaintenanceModeException {
			...
			if (content.getUUID() == null) {
				try {
					...
					if (!(content instanceof HIView))
						...
						content.setUUID(
								UUID.nameUUIDFromBytes(storageManager.getOriginal((HIView) content)).toString());
					em.persist(content);
					em.flush();
					utx.commit();
				} catch (NotSupportedException | SystemException | RollbackException | HeuristicMixedException
	...
	@WebMethod
	public synchronized HIQuickInfo getBaseQuickInfo(@WebParam(name = "baseID") long baseID)
			throws HIPrivilegeException, HIEntityNotFoundException, HIMaintenanceModeException {
	...
	@WebMethod
	public synchronized HIQuickInfo getBaseQuickInfoByUUID(@WebParam(name = "uuid") String uuid)
			throws HIPrivilegeException, HIEntityNotFoundException, HIMaintenanceModeException, HIParameterException {
	...
	public synchronized HIQuickInfo createContentQuickInfo(HIBase content) {
		...
		if (!(content instanceof HIURL) && !(content instanceof HILightTable)) {
			...
			if (record != null) {
				...
				// for objects respect user wishes --> fall back to HIClassic
				// title if no pref exists
				EntityManager em = emf.createEntityManager();
				if (content instanceof HIObject) {
					...
					if (curProject != null) {
						...
						fieldPref = MetadataHelper.findPreference(content.getProject(),
								"admin.preview.objectTitleField");
					}
					...
					if (fieldPref != null) {
						title = record.findValue(fieldPref.getValue().split("\\.")[0],
								fieldPref.getValue().split("\\.")[1]);
					}
					// fallback to dc.title
					if (title == null)
						title = record.findValue("dc", "title");
					// fallback to view title of default view (if instanceof
					// HIView and available)
					if (title == null) {
						HIFlexMetadataRecord titleRecord = null;
						HIObject object = (HIObject) content;
						if (object.getViews().size() > 0) {
							HIView defView = null;
							if (object.getDefaultView() != null && object.getDefaultView() instanceof HIView)
								defView = (HIView) object.getDefaultView();
							if (defView == null && object.getViews().get(0) instanceof HIView)
								defView = (HIView) object.getViews().get(0);
							if (defView != null && curProject != null)
								titleRecord = MetadataHelper.getDefaultMetadataRecord(defView,
										curProject.getDefaultLanguage());
						}

						if (titleRecord != null)
							title = titleRecord.findValue("HIBase", "title");
					}
				}
				...
				if (content instanceof HIObject) {
					...
					if (object.getDefaultView() != null) {
						if (object.getDefaultView() instanceof HIInscription) {
							if (curProject != null) {
								record = MetadataHelper.getDefaultMetadataRecord(object.getDefaultView(),
										curProject.getDefaultLanguage());
							} else {
								record = MetadataHelper.getDefaultMetadataRecord(object.getDefaultView(),
										object.getProject().getDefaultLanguage());
							}
							needsPreviewText = true;
						}
					} else {
						if (object.getViews().size() > 0) {
							if (object.getSortedViews().get(0) instanceof HIInscription) {
								if (curProject != null) {
									record = MetadataHelper.getDefaultMetadataRecord(object.getViews().get(0),
											curProject.getDefaultLanguage());
								} else {
									record = MetadataHelper.getDefaultMetadataRecord(object.getViews().get(0),
											object.getProject().getDefaultLanguage());
	...
	@WebMethod
	public synchronized boolean moveToGroup(@WebParam(name = "baseID") long baseID,
			@WebParam(name = "fromGroupID") long fromGroupID, @WebParam(name = "toGroupID") long toGroupID)
					throws HIMaintenanceModeException {
	...
	@WebMethod
	public synchronized boolean copyToGroup(@WebParam(name = "baseID") long baseID,
			@WebParam(name = "groupID") long groupID) throws HIParameterException, HIMaintenanceModeException {
	...
	@WebMethod
	public synchronized boolean moveToTrash(@WebParam(name = "baseID") long baseID)
			throws HIParameterException, HIMaintenanceModeException {
		...
		// if element was an object, remove all views, inscriptions and layers
		// from groups, and their links
		if (base instanceof HIObject) {
			...
			// if element was a view, remove all layers from groups, and their
			// links
			if (base instanceof HIView) {
				for (HILayer layer : ((HIView) base).getLayers()) {
					for (HIGroup group : findGroups(em, layer)) {
	...
	@WebMethod
	public synchronized boolean removeFromGroup(@WebParam(name = "baseID") long baseID,
		...
		if (group.getType() != HIGroup.GroupTypes.HIGROUP_REGULAR
				&& group.getType() != HIGroup.GroupTypes.HIGROUP_TAG) {
			return false;
		}
	...
	private synchronized long countGroups(HIBase base) {
		...
		long size = (Long) em.createQuery("SELECT count(g) FROM HIGroup g WHERE g.contents=:base AND g.type=:type")
				.setParameter("base", base).setParameter("type", HIGroup.GroupTypes.HIGROUP_REGULAR).getSingleResult();
	...
	private synchronized boolean addToImportGroup(HIBase base) {
		if ((base instanceof HILayer) || (base instanceof HIView) || (base instanceof HIInscription)) {
			return false;
		}
	...
	private synchronized boolean addToGroup(HIBase base, HIGroup group) {
		...
		try {
			...
			if (!group.equals(importGroup) && group.getType() != HIGroup.GroupTypes.HIGROUP_TAG) {
				/*
				 * if (base instanceof HIObjectContent) {
				 * removeFromGroup(((HIObjectContent) base).getObject(),
				 * importGroup); } else if (base instanceof HILayer) {
				 * removeFromGroup(((HILayer) base).getView().getObject(),
				 * importGroup); } else
				 */if (!(base instanceof HIGroup)) {
					removeFromGroup(base, importGroup);
				}
	...
	private synchronized void removeLinks(HIBase base) {
		...
		try {
			...
			for (long tagID : tagIDList) {
				...
				for (int i = 0; i < tag.getContents().size(); i++)
					if (tag.getContents().get(i).getId() == base.getId())
						index = i;
				if (index >= 0) {
			...
			utx.commit();
			base = em.find(HIBase.class, base.getId());
		} catch (HIPrivilegeException | HIEntityNotFoundException | NotSupportedException | SystemException
				| RollbackException | HeuristicMixedException | HeuristicRollbackException | SecurityException
				| IllegalStateException ex) {
			m_logger.log(Level.SEVERE, null, ex);
		}
		...
		List<HILayer> layers = em.createQuery("SELECT l FROM HILayer l WHERE l.link=:base").setParameter("base", base)
				.getResultList();
	...
	// DEBUG
	@WebMethod
	public synchronized List<HIQuickInfo> simpleSearch(@WebParam(name = "text") String text,
			@WebParam(name = "language") String language) throws HIMaintenanceModeException {
	...
	@WebMethod
	public synchronized List<HIQuickInfo> fieldSearch(@WebParam(name = "fields") List<String> fields,
			@WebParam(name = "contents") List<String> contents, @WebParam(name = "language") String language)
	...
	// -------------------------
	@WebMethod
	public HIFlexMetadataRecord createFlexMetadataRecord(@WebParam(name = "baseID") long baseID,
		...
		try {
			base = (HIBase) em.createQuery("SELECT b FROM HIBase b WHERE b.id=:id AND b.project=:project")
					.setParameter("id", baseID).setParameter("project", curProject).getSingleResult();
		} catch (NoResultException e) {
	...
	// internal method
	private HIFlexMetadataRecord createFlexMetadataRecord(EntityManager em, HIBase base, String language) {
		...
		} else if (base instanceof HIInscription || base instanceof HILightTable || base instanceof HIText) {
			record.addEntry(new HIKeyValue("HIBase.title", ""));
		...
		em.flush();
		/*
		 * utx.commit(); } catch (NotSupportedException | SystemException |
		 * RollbackException | HeuristicMixedException |
		 * HeuristicRollbackException | SecurityException |
		 * IllegalStateException ex) { m_logger.log(Level.SEVERE, null, ex); }
		 */
	...
	@WebMethod
	public List<HIFlexMetadataRecord> getFlexMetadataRecords(@WebParam(name = "baseID") long baseID)
	...
	@WebMethod
	public boolean updateFlexMetadataRecords(@WebParam(name = "records") List<HIFlexMetadataRecord> records)
			throws HIParameterException, HIMaintenanceModeException {
	...
	// DEBUG: optimize this
	@WebMethod
	public boolean updateFlexMetadataRecord(@WebParam(name = "record") HIFlexMetadataRecord record)
			throws HIParameterException, HIMaintenanceModeException {
	...
	private void setRole(HIUser user, HIProject project, HIRole.HI_Roles type) {
		...
		try {
			...
			if (role != null) {
				...
				// create role for user, effectively adding the user to the
				// project
	...
	private void addPrometheusTemplate(HIProject project) {
		...
		try {
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "KünstlerIn",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Artist",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 3 title
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Titel",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Title",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 3 location
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Standort",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Location",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 4 discoveryplace
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Fundort",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Discoveryplace",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 5 genre
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Gattung",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Genre",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 6 material
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Material",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Material",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 7 keyword
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Schlagwörter",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Keyword",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 8 description
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Beschreibung",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Description",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 9 date
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Datierung",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Date",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 10 credits
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Bildnachweis",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Credits",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 13 addition
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Zusatz",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Addition",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 14 annotation
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Bermerkung",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Annotation",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 16 classification
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Einordnung",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Classification",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 19 depository
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Aufbewahrungsort",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Depository",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 22 discoverycontext
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Fundkontext",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Discoverycontext",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 23 epoch
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Epoche",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Epoch",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 25 inscription
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Inschrift",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Inscription",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 26 institution
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Institution",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Institution",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 27 inventory_no
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Inventarnummer",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Inventory No",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 28 isbn
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "ISBN",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "ISBN",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 30 literature
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Literatur",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Literature",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Ort Gebäude",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Location Building",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Herstellungsort",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Manufacture Place",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 33 origin
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Herkunft",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Origin",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 34 photographer
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Fotograf",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Photographer",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 35 signature
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Bildsignatur",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Signature",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Bildrecht Foto",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Rights Reproduction",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// new2 rights_work
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Bildrecht Werk",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Rights Work",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 37 source_url
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Datensatz in Quelldatenbank",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Source URL",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 38 subtitle
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Untertitel",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Subtitle",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 43 technique
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Technik",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Technique",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 49 detail
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Detail",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Detail",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 54 size
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "Maße",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "Size",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
			// 55 pid
			...
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("de", "PID",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			prTemplate.getEntries().get(prTemplate.getEntries().size() - 1).getDisplayNames()
					.add(new HIFlexMetadataName("en", "PID",
							prTemplate.getEntries().get(prTemplate.getEntries().size() - 1)));
			em.persist(prTemplate);
			em.flush();
	...
	public static String getMD5HashString(byte[] inputData) {
		...
		try {
			...
			for (int i = 0; i < digest.length; i++) {
				curNum = digest[i]; // make sure current byte of digest hex is
									// unsigned
				if (curNum < 0) {
					curNum = curNum + 256;
				}
