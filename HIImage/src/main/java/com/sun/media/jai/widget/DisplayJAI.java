...
/**
 * A Java<sup>TM</sup> Foundation Classes {@link JPanel} which is able to
 * contain an image. The image and its container may have different sizes. The
 * image may also be positioned within the container.
 *
 * <p>
 * This class extends <code>JPanel</code> in order to support layout management.
 * Image tiling is supported as of version 1.3 of the Java<sup>TM</sup> 2
 * Platform via
 * {@link Graphics2D#drawRenderedImage(RenderedImage,AffineTransform)}.
 * </p>
 *
 * <p>
 * <i>This class is not a committed part of the Java<sup>TM</sup> Advanced
 * Imaging API per se. It might therefore not be supported by JAI
 * implementations other than that of Sun Microsystems, Inc.</i>
 * </p>
 *
...
public class DisplayJAI extends JPanel implements MouseListener, MouseMotionListener {
	 ...
	 * <code>originX</code> and <code>originY</code> are set to the parameter
	 * valus <code>x</code> and <code>y</code>, respectively. {@link #repaint()}
	 * is invoked after the origin values are changed.
	 *
	 * @param x
	 *            image origin abscissa.
	 * @param y
	 ...
	 * The preferred size is set such that its width is the image width plus the
	 * left and right insets and its height is the image height plus the top and
	 * bottom insets. {@link #revalidate()} and {@link #repaint()} are invoked
	 ...
	 *
	 * <p>
	 * If the current image is <code>null</code>, the rectangle
	 * <code>new Rectangle(0,0,{@link #getWidth()},{@link #getHeight()})</code>
	 * is filled with the background color returned by {@link #getBackground()}.
	 * </p>
	 *
	 * <p>
	 * If the current image is non-<code>null</code>, the rectangle returned by
	 * {@link Graphics2D#getClipBounds()} is filled with the background color
	 * and the image is drawn using
	 * {@link Graphics2D#drawRenderedImage(RenderedImage,AffineTransform)} at
	 * the location <code>({@link #getInsets()}.left+originX,
	* {@link #getInsets()}.right+originY)</code>.
	...
	/**
	 * An empty method which should be overridden by subclasses which respond to
	 * mouse presses.
	 */
