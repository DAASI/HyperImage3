...
function hookAndShowDateSlider (divName, selectorType) {
	...
	if (reader.timeslider.startDate && reader.timeslider.endDate) {
            ...
            reader.timeslider.obj = $("#dateSlider").dateRangeSlider(
                {
                    ...
                    wheelMode: "zoom",
                    bounds:{
                        min: new Date(reader.timeslider.startDate.year(), 0, 1),
                        max: new Date(reader.timeslider.endDate.year(), 11, 31)
                    },
                    defaultValues: {
                        min: new Date(reader.timeslider.startDate.year(), 0, 1),
                        max: new Date(reader.timeslider.endDate.year(), 11, 31)
            ...
            reader.timeslider.obj = $("#dateSlider").dateRangeSlider(
				{
                    ...
                    wheelMode: "zoom",
                    bounds:{
                        min: new Date(reader.timeslider.startDate.year(), reader.timeslider.startDate.month(), reader.timeslider.startDate.date()),
                        max: new Date(reader.timeslider.endDate.year(), reader.timeslider.endDate.month(), reader.timeslider.endDate.date())
                    },
                    defaultValues: {
                    	min: new Date(reader.timeslider.startDate.year(), reader.timeslider.startDate.month(), reader.timeslider.startDate.date()),
                        max: new Date(reader.timeslider.endDate.year(), reader.timeslider.endDate.month(), reader.timeslider.endDate.date())
					...
					scales: [{
						...
						next: function(value){
							var interval = Math.ceil((reader.timeslider.endDate.year() - reader.timeslider.startDate.year())/7);
							var next = new Date(value);
        ...
        $(selectorQuery).each(function() {
            ...
            $.each(this.attributes,function(i,a){
                if (a.name.match(attributeQuery)) {
                    var filterTime = moment(a.value, reader.timeslider.timeFormat);
                    if (filterTime.isValid()) {
                        if (filterTime.isBefore(reader.timeslider.startDate) || filterTime.isAfter(reader.timeslider.endDate)) {
                            $(selector).fadeTo(500, 0.5);
                        } else {
                            $(selector).fadeTo(500, 1);
                        }
		...
		// set default range selection
        //var minDateOfObjs = new Date(reader.timeslider.startDate.year(), reader.timeslider.startDate.month(), reader.timeslider.startDate.date());
        //var maxDateOfObjs = new Date(reader.timeslider.endDate.year(), reader.timeslider.endDate.month(), reader.timeslider.endDate.date());
        //reader.timeslider.obj.dateRangeSlider("values", minDateOfObjs, maxDateOfObjs);
	}
}
